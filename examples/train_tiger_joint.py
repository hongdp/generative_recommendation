...
from models.tiger_joint import TIGERJointModel
...
def main():
    ...
    parser.add_argument("--latent_dim", type=int, default=256, help="RQVAE Latent dimension.")
    ...
    # 3. Setup Model
    print("Initializing TIGERJointModel...")
    ...
    # Inject PCA initialization for codebooks
    codebooks_init = np.stack([init_E1, init_E2, init_E3])
    params["codebooks"] = jnp.array(codebooks_init)
    
    # Optional: also initialize the token embeddings with codebooks if desired?
    ...
    # Inject PCA into decoder projection
    # W_proj_init is [768, latent_dim] => decoder needs [latent_dim, 768]
    # W_proj_init.T is [latent_dim, 768]
    # nn.Dense has kernel shape [in_features, out_features] => [latent_dim, 768]
    params["decoder"]["kernel"] = jnp.array(W_proj_init.T)
    ...
    def train_step(params, opt_state, batch_enc, batch_dec_in, batch_dec_tar, batch_Z_tar, dropout_key):
        def loss_fn(p):
            ...
            # Reconstruction loss
            loss_recon = jnp.mean(jnp.sum((x_recon - batch_Z_tar)**2, axis=-1))
            
            # Total loss
            total_loss = loss_token + loss_recon + codebook_loss + 0.25 * commitment_loss
            return total_loss, (loss_token, loss_recon, codebook_loss, commitment_loss)

        (loss, aux), grads = jax.value_and_grad(loss_fn, has_aux=True)(params)
        loss_token, loss_recon, codebook_loss, commitment_loss = aux
        updates, opt_state = optimizer.update(grads, opt_state, params)
        params = optax.apply_updates(params, updates)
        return params, opt_state, loss_token, loss_recon, codebook_loss, commitment_loss
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        for i in range(0, num_samples, batch_size):
            ...
            params, opt_state, loss_token, loss_recon, codebook_loss, commitment_loss = train_step(
                params,
                opt_state,
                jnp.array(batch_enc),
            ...
            epoch_loss_cb += codebook_loss
            epoch_loss_commit += commitment_loss
            num_batches_processed += 1
            global_step += 1

            if writer is not None and global_step % 10 == 0:
                writer.add_scalar("Loss/token", float(loss_token), global_step)
                writer.add_scalar("Loss/recon", float(loss_recon), global_step)
                writer.add_scalar("Loss/codebook", float(codebook_loss), global_step)

        elapsed = time.time() - start_time
        avg_loss_token = float(epoch_loss_token) / num_batches_processed
        avg_loss_recon = float(epoch_loss_recon) / num_batches_processed
        print(f"Epoch {epoch:02d}/{epochs} | Token Loss: {avg_loss_token:.4f} | Recon: {avg_loss_recon:.4f} | CB: {float(epoch_loss_cb)/num_batches_processed:.4f} | Time: {elapsed:.2f}s")
        
        if writer is not None:
            writer.add_scalar("Loss/train_epoch_token", avg_loss_token, global_step)
            writer.add_scalar("Loss/train_epoch_recon", avg_loss_recon, global_step)

        # Reassign SIDs and Regenerate Dataset
        E1_curr = np.array(params["codebooks"][0])
        E2_curr = np.array(params["codebooks"][1])
        E3_curr = np.array(params["codebooks"][2])
        
        # We need W_proj to reassign codes correctly, which is the decoder weights now
        W_proj_curr = np.array(params["decoder"]["kernel"]).T # [768, latent_dim]
        c_table_new, E1_new, E2_new, E3_new = reassign_sids(Z_frozen, E1_curr, E2_curr, E3_curr, W_proj_curr)
        c_table_new[0] = [0, 0, 0]
        
        # Inject revived codes back into model parameters
        new_codebooks = np.array(params["codebooks"])
        new_codebooks[0] = E1_new
        new_codebooks[1] = E2_new
        new_codebooks[2] = E3_new
        params["codebooks"] = jnp.array(new_codebooks)
        
        # Keep token embeddings synced
