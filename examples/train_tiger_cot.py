...
def preprocess_seq2seq_training_data(inputs, targets, semantic_ids, K, start_token):
    ...
    decoder_inputs = np.zeros((batch_size, 4), dtype=np.int32)
    ...
    for i in range(batch_size):
        ...
        # decoder inputs: [start, c1+1, c2+K+1, c3+2*K+1]
        decoder_inputs[i, 1] = c1 + 1
        decoder_inputs[i, 2] = c2 + K + 1
        decoder_inputs[i, 3] = c3 + 2 * K + 1
...
def main():
    ...
    parser.add_argument("--tb_log_dir", type=str, default="./data/tensorboard/tiger_cot_steam", help="TensorBoard log directory.")
    ...
    if args.checkpoint_dir == "./data/tiger_seq2seq_checkpoints":
        args.checkpoint_dir = f"./data/tiger_cot_{dataset}_checkpoints"
    ...
    print(f"--- Training TIGER CoT Model on {args.dataset.upper()} ---")
    ...
    # Z_frozen is expected to be [num_items+1, 768] (with index 0 as padding)
    ...
    # 3. Setup Model
    print("Initializing TIGER CoT Model...")
    model = TIGERCoTModel(
    ...
    dummy_dec = jnp.zeros((1, 4), dtype=jnp.int32)
    ...
    def train_step(params, opt_state, batch_enc, batch_dec_in, batch_dec_tar, batch_z_tar, dropout_key):
        def loss_fn(p):
            ...
            # Cross entropy loss across the first 3 positions of the decoder for discrete routing
            loss_token = jnp.mean(optax.softmax_cross_entropy_with_integer_labels(logits[:, :3, :], batch_dec_tar))
            
            # MSE loss on the 4th decoder step continuous embedding
            loss_embed = jnp.mean(jnp.sum((e_outs[:, 3, :] - batch_z_tar)**2, axis=-1))
            
            total_loss = loss_token + loss_embed
            return total_loss, (loss_token, loss_embed)
