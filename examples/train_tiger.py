...
def main():
    ...
    parser.add_argument("--level_loss_weights", type=str, default="",
                        help="Comma-separated per-level CE weights, e.g. '2,1,0.5'. Empty = uniform.")
    parser.add_argument("--user_buckets", type=int, default=0,
                        help="If >0, prepend a hashed user-id token (this many buckets) after the start token, as in the TIGER paper.")
    parser.add_argument("--lr_schedule", type=str, default="constant", choices=["constant", "invsqrt", "cosine"],
                        help="LR schedule. invsqrt = warmup then inverse-sqrt decay (TIGER paper); cosine = warmup then cosine decay.")
    parser.add_argument("--warmup_steps", type=int, default=1000, help="Warmup steps for invsqrt/cosine schedules.")
    parser.add_argument("--eval_every", type=int, default=1, help="Run validation every N epochs (LIGER: 10 for long runs).")
    ...
    U = args.user_buckets  # 0 = no user token
    # Vocab: pad 0 | level i: [i*K+1, (i+1)*K] | start: L*K+1 | user buckets: [L*K+2, L*K+2+U)
    vocab_size = L * K + 2 + U
    start_token = L * K + 1

    level_weights = None
    if args.level_loss_weights:
        level_weights = [float(w) for w in args.level_loss_weights.split(",")]
        if len(level_weights) != L:
            raise ValueError(f"--level_loss_weights needs {L} values, got {len(level_weights)}")
        print(f"Per-level CE loss weights: {level_weights}")
    ...
    train_tokens_in, train_tokens_tar = preprocess_decoder_only_training_data(
    ...
    val_tokens_in = sequence_to_decoder_only_tokens(val_in, semantic_ids, K, start_token, num_levels=L)

    test_dataset = loader.get_split("test", max_len=max_len, format_type="index")
    test_in, test_tar = test_dataset.to_numpy()
    test_tokens_in = sequence_to_decoder_only_tokens(test_in, semantic_ids, K, start_token, num_levels=L)

    if U > 0:
        # Prepend a hashed user token right after `start` ([start, u, history...]),
        # mirroring the TIGER paper. Sample order matches build_sequence_data.
        from datasets.base import build_sequence_user_ids

        def user_tokens(split):
            uids = build_sequence_user_ids(loader.user_history, split)
            return np.array([L * K + 2 + (int(u) % U) for u in uids], dtype=np.int32)

        u_train, u_val, u_test = (user_tokens(s) for s in ("train", "val", "test"))
        # Train stream: insert u into inputs at position 1; mask the new
        # start->u prediction position in targets (user token is not learnable
        # from a bare start token — target 0 is excluded by the loss mask).
        train_tokens_in = np.insert(train_tokens_in, 1, u_train, axis=1)
        train_tokens_tar = np.insert(train_tokens_tar, 0, 0, axis=1)
        val_tokens_in = np.insert(val_tokens_in, 1, u_val, axis=1)
        test_tokens_in = np.insert(test_tokens_in, 1, u_test, axis=1)
        print(f"User token enabled: {U} buckets, streams extended by 1 position")
    ...
    model = TIGERModel(
        ...
        max_sequence_len=L * max_len + L + 1 + (1 if U > 0 else 0),
        attn_dropout_rate=args.dropout_rate,
        linear_dropout_rate=args.dropout_rate,
    )

    key = jax.random.PRNGKey(42)
    dummy_seq = jnp.zeros((1, L * max_len + L + (1 if U > 0 else 0)), dtype=jnp.int32)
    variables = model.init(key, dummy_seq)
    params = variables["params"]

    # 4. Setup Optimizer
    if args.lr_schedule == "invsqrt":
        # Linear warmup to peak, then inverse-sqrt decay (TIGER-paper style).
        w = args.warmup_steps
        peak = args.learning_rate
        lr = lambda step: jnp.where(
            step < w, peak * step / w, peak * jnp.sqrt(w / jnp.maximum(step, 1)))
        print(f"LR schedule: invsqrt (warmup {w} steps, peak {peak})")
    elif args.lr_schedule == "cosine":
        steps_per_epoch = max(1, (len(train_tokens_tar) // args.batch_size))
    ...
    # Per-position level weights: target position j predicts flat-stream position
    # j+1, whose Semantic-ID level is j % L (the stream after `start` cycles
    # through levels 1..L for every item).
    seq_len = L * max_len + L + (1 if U > 0 else 0)
    if level_weights is not None:
        off = 1 if U > 0 else 0
        pos_weights = jnp.array([level_weights[max(j - off, 0) % L] for j in range(seq_len)], dtype=jnp.float32)
    else:
        pos_weights = jnp.ones((seq_len,), dtype=jnp.float32)
    ...
    def train_step(params, opt_state, batch_inputs, batch_targets, dropout_key):
        def loss_fn(p):
            ...
            # scaled by the per-level weights (uniform by default).
            mask = (batch_targets != 0).astype(jnp.float32) * pos_weights[None, :]
            # Cross entropy loss
    ...
    def beam_search_decode(params, batch_inputs, B=10):
        return beam_search_decode_decoder_only(params, batch_inputs, predict_next_token, K, B=B, num_levels=L)
    ...
    model_desc += f" L{args.num_levels}xK{args.num_codes}"
    if args.level_loss_weights:
        model_desc += f" lw[{args.level_loss_weights}]"
    results_row = (
